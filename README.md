...
BuyLink (カートアイコンの購入リンク) が出ない画面（プレイリストのリストビューなど）でも、BuyLinkが設定されている楽曲は分かるように表示を行う。  
...
稀にBuyLinkで設定せず楽曲の説明文に入手先のURLを記載している楽曲があるため、  
Likes画面の「List」表示に限り、楽曲の説明文にURLが含まれている場合、BuyLinkと同じエリアにリンクを並べ、曲の詳細ページに飛ばなくても見れるようにする。  

画面幅の都合上、Feedなど他の画面では出ない。  
また、説明文にURLが沢山ある場合に入り切らない分は省略されて表示される。

<details>
<summary> 詳細</summary>

説明文に複数のURLが含まれる場合、再生数などの表示に被らない分だけ表示し、入り切らない分は「N more」という表記にまとめる。  
購入リンク（BuyLink）と同一のURLは表示しない。また、以下の宛先のリンクは大半のトラックの説明文に定型文として入っており個別に目立たせる価値が薄いため表示しない。
- x.com
- twitter.com
- soundcloud.com
- on.soundcloud.com
