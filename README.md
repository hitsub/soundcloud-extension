...
Likes画面（`/you/likes`・`/{ユーザー名}/likes`）の「List」表示で、楽曲の説明文にURLが含まれている場合、そのドメイン名のリンクボタンをMoreボタンの右（購入リンクのカートアイコンがある場合はその右）に表示する。クリックすると新しいタブでそのURLが開く。  
同じ「List」表示のDOMを使うFeedや各ユーザープロフィールでは横幅が狭くこのボタンを表示する余白が無いため、Likes画面限定にしている。
